...
system_create_user() {
  ...
  mv "${PROJECT_ROOT}"/smartatendimentopro.zip /home/deploy/
EOF
...
system_unzip_whaticket() {
  print_banner
  printf "${WHITE} 💻 Descompactando o smartatendimentopro.zip...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2

  sudo su - deploy <<EOF
  unzip smartatendimentopro.zip
EOF
...
system_mysql_install() {
  print_banner
  printf "${WHITE} 💻 Instalando servidor MySQL...${GRAY_LIGHT}"
  printf "\n\n"
